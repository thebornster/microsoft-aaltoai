...
**Every P0, P1, and P2 item from the design doc's priority list is now done.** P0/P1 are live-demoed end-to-end (see below); P2's four items (eval suite, Teams card, console polish, real bulletin PDF) are all complete. All tests green (40, unchanged by the PDF swap). There is no queued next unit of work — pick from these only if asked, or ask the user what's next:

1. Foundry Local semantic fallback is P3 and explicitly optional — do not start it unless asked directly.
2. The Teams webhook has never been fired against a real Power Automate Workflows URL (no `RAJA_TEAMS_WEBHOOK_URL` configured in this environment) — it's unit-tested with a mocked `httpx.post`, not live-verified against real Teams. Only chase a live Teams test if a judge specifically asks to see the card render, and you'd need a real Workflows webhook URL from the user first.
3. Otherwise: general demo-day hardening (rehearsing the live run, checking timing) is the only thing left that isn't a discrete code task.
...
- Real `demo/bulletin_A19.pdf` done: hand-built a minimal valid single-page PDF (raw PDF 1.4 objects/xref/trailer written by a one-off script, not committed — no `reportlab`/`fpdf` dependency added) containing the exact same injected bulletin text as `demo/bulletin_A19.txt`. Verified with `file` (reports "PDF document, version 1.4, 1 pages") and a `pypdf.PdfReader` extraction round-trip that matches the `.txt` content exactly. **`gateway/backends.py`'s `search_supplier_docs` still reads from `bulletin_A19.txt`, not the new `.pdf`** — the `.pdf` is a real demo/judge-facing asset (something you can actually open and show), the `.txt` stays the single source of truth the gateway parses at runtime; wiring the gateway to parse PDF bytes was judged out of scope (would need a PDF text-extraction dependency, contradicting "no PDF-writer dependency" and adding real complexity for a demo-only cosmetic need). If a judge specifically wants the gateway itself to ingest the `.pdf` bytes, that's new scope — ask before doing it.
