...
async function resume(){document.querySelector("#continue").disabled=true;line("→ Agent: retrying the exact approved call");const out=await call("post_supplier_ticket",reviewArgs,true);if(out.result){line("✓ ALLOW AFTER APPROVAL · supplier backend invoked","ok");statusEl.textContent="COMPLETE";document.querySelector("#approval").style.display="none";line("Done. Open the Dashboard tab to inspect the full lineage.","ok")}else{line("✕ Resume rejected: "+esc(out.error),"bad");statusEl.textContent="ERROR"}}
</script></body></html>""")
